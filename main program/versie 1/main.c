...
void LED_DEBUG(){
	GPIOA->MODER |= GPIO_MODER_MODER5_0; //output internal LED
...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  char msgBuf[MSGBUFSIZE];
	  snprintf(msgBuf, MSGBUFSIZE, "Distance in cm: %ld\n", distance);
	  HAL_UART_Transmit(&huart2, (uint8_t *)msgBuf, strlen(msgBuf), HAL_MAX_DELAY);

	  Right_Wheel_Forward_Rotation(60);
	  Left_Wheel_Forward_Rotation(60);
	  if (distance <= 10){
		  Stop_Wheels();
	  }
  }
