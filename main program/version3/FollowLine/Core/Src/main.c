...
const double Ki = 0.007;		//change for rate of speed up
...
void ReadSerial(void)
{
	if(HAL_UART_Receive(&huart2, (uint8_t *)&receivedData, 1, 0) == HAL_OK)
	{
		HAL_UART_Transmit(&huart2, (uint8_t *)receivedData, strlen(receivedData), HAL_MAX_DELAY);
	      ...
	      // Check if the conversion was successful and the value is within the desired range
	      if (receivedValue != 0 || strcmp(receivedData, "0") == 0)
	      {
//	    	HAL_UART_Transmit(&huart2, (uint8_t *)receivedValue, strlen(receivedValue), HAL_MAX_DELAY);
...
void Servo_Controller(double PID_output){
	int value_right = RIGHT_MOTOR_FORWARD - PID_output;
	int value_left = LEFT_MOTOR_FORWARD + PID_output;
	int correct_right = value_right - 16;	//needed to maintain same straight speed as left motor.
	TIM2->CCR1 = (correct_right/2);
	TIM2->CCR2 = (value_left/2);
}
