...
void Servo_Clockwise_Rotation(int value);	//value = percentage speed.
void Servo_Counter_Clockwise_Rotation(int value);	//value = percentage speed.
...
int main(void)
{
  ...
  while (1)
  {
	  Servo_Clockwise_Rotation(100);
	  Servo_Counter_Clockwise_Rotation(100);
	  HAL_Delay(1000);
  }
}
...
void Pin_Config(){
	...
	//setting MODER for PA1 and PA0 (alternating function)
...
//	GPIOB->MODER = (GPIOB->MODER & ~(GPIO_MODER_MODER5)) | (0b01 << GPIO_MODER_MODER5_Pos); /output
...
void Servo_Clockwise_Rotation(int value) {
	//values from the clockwise roation section in datasheet.
	value = map(value, 0, 100, 1470, 1280);
	TIM2->CCR1 = (value/2);
}

void Servo_Counter_Clockwise_Rotation(int value){
